...
| 12 | 12 | 我跟 AI 實際工作的樣子 | 6 | `#/presenter/12/0` |
...
| 12 | 承接 14：發現不對之後我實際上怎麼做 —— 真實對話紀錄（原話，未修飾）→ 看／不對／指出問題／要它修／再看／再修 → 可以罵，但要罵得有規格 → 收在 THE PROMPT IS THE CONVERSATION |
...
- Scene11 會用 `import.meta.glob` 自動掃描根目錄所有 `cibar-*` 圖片組成展示牆；
  ...
  沒補的會顯示「情境模組 N」，不會把檔名投到畫面上。
  成果展示只留「刑事警察局反詐 AR APP」一個案例（對外一律用這個名稱，
  `cibar-*` 只是檔名／內部識別，不上畫面），旁邊那張卡是「原本不是我的專業」，
  不是第二個專案 —— 這一頁不列工作清單。
